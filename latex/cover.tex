...
\Title{\selectlanguage{portuguese}Integração de Dados de Sensores e Tolerância a Falhas em Ambientes Inteligentes}
...
\Title{\selectlanguage{english}Sensor Data Integration and Management of Smart Environments}
...
%\SecondSupervisor{Rui Aguiar}{catedrático c/agregação}{do Departamento de Eletrónica, Telecomunicações e Informática}{da Universidade de Aveiro}
...
\Acknowledgements{\selectlanguage{portuguese}Agradecimentos.
}
...
\Abstract{Palavras Chave}{Internet das Coisas, Redes de Sensores, Automação, Processamento de Eventos Complexos.}
         {Resumo}{\selectlanguage{portuguese}Esta dissertação enquadra-se no projecto SmartLighting e tem como objectivo procurar criar uma solução energeticamente eficiente para edifícios e espaços inteligentes. Numa primeira fase, esta dissertação visa apresentar uma revisão das existentes soluções automação de edifícios e posteriormente propôr uma solução, baseada em princípios da Internet das Coisas e em sistemas de processamento complexo de eventos, capaz de criar um ambiente inteligente, autónomo e resiliente a falhas. O foco será na criação de um software leve para ser colocado em dispositivos com pouca capacidade de processamento de modo a poderem, não só ser um meio para comunicação com dispositivos inteligentes, mas também habilitados para oferecer capacidades de automação.
 }
...
\Abstract{Keywords}{Internet of Things, Wireless Sensor Networks, Automation, Complex Event Processing.}
         {Abstract}{\selectlanguage{english}This dissertation fits it self within the SmartLighting project, with the objective of creating an energy efficient solution for buildings and smart spaces. This dissertation, in a first stage, aims to review the current solutions of building automation and, later, propose a solution, based on Internet of Things (IoT) principles and Complex Event Processing (CEP) systems, able to create a failure safe, autonomous and smart environment. The focus will be on creating lightweight software to be deployed in constrained devices to act as gateways with automation capabilities and means to connect to smart devices.
     }

%%% Edit the Jury President information.
%% First Member will be the President of the Jury
\JuriMember{A decidir}{A decidir}
